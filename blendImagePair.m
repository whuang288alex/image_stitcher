...
%BLENDIMAGEPAIR
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            weighted_mask_s = bwdist(~binary_mask_s,'euclidean');
            weighted_mask_d = bwdist(~binary_mask_d,'euclidean');
            temp = binary_mask_s | binary_mask_d;
            channel_out(temp) = (S(temp).*weighted_mask_s(temp) + D(temp).*weighted_mask_d(temp))./(weighted_mask_s(temp) + weighted_mask_d(temp)); 
        end
